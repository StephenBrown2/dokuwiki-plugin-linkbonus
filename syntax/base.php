 ...
 */
class syntax_plugin_linkbonus_base extends DokuWiki_Syntax_Plugin {
    ...
    // this should allow the plugin to accept formatting syntax, eg.: italics.
...
//      }
    ...
    function connectTo($mode) { 
      ...
      $otherpart = '[^\]]+'; 
      $REGEX     = 
      '\[\['. $linkpart. '\|'. $namepart. '\|'. $otherpart. '\]\]';
      $this->Lexer->addSpecialPattern(
        $REGEX,$mode,'plugin_linkbonus_base'); 
      }
    ...
    function handle($match, $state, $pos, &$handler) {
        ...
        /* Recover base of server */
        $link['url'] = substr($link['url'], 1); // remove leading '@'
        $link['url'] = DW_common_linkbonus::_getServerBase().  $link['url'];
